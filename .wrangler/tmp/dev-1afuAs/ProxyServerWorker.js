...
var Evaluator = class _Evaluator {
	...
	promises = [];
	evaluate(value) {
		const payload = RpcPayload.forEvaluate(this.stubs, this.promises);
		try {
					...
					case "pipeline": {
						...
						const addStub = /* @__PURE__ */ __name((hook2) => {
							if (isPromise) {
								...
								const stub = new RpcPromise(hook2, []);
								this.stubs.push(stub);
								return stub;
							}
						}, "addStub");
...
async function evaluateMediaBinding(headers, stream, binding) {
	const inputOptions = JSON.parse(headers.get("x-cf-media-input-options"));
	const outputOptions = JSON.parse(headers.get("x-cf-media-output-options"));
		...
		try {
			if (isJSRPCBinding(request)) {
				...
				if (isSpecialCaseMediaBindingRequest(originalHeaders)) {
					return evaluateMediaBinding(originalHeaders, request.body, fetcher);
				}
...
// .wrangler/tmp/bundle-v6USag/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-v6USag/middleware-loader.entry.ts
