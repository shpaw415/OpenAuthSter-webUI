...
var StubHook = class {
	...
	//   (no overlapping).
	stream(path, args) {
		const hook = this.call(path, args);
		const pulled = hook.pull();
		let promise;
...
var RpcStub = class _RpcStub extends RpcTarget {
	...
	constructor(hook, pathIfPromise) {
		...
		const func = /* @__PURE__ */ __name(() => {}, "func");
		func.raw = this;
	...
	map(func) {
		const { hook, pathIfPromise } = this[RAW_STUB];
...
function unwrapStubTakingOwnership(stub) {
	const { hook, pathIfPromise } = stub[RAW_STUB];
...
var RpcPayload = class _RpcPayload {
	...
	static deepCopyFrom(value, oldParent, owner) {
		const result = new _RpcPayload(null, "owned", [], []);
	...
	ensureDeepCopied() {
		if (this.source !== "owned") {
			const dupStubs = this.source === "params";
			this.hooks = [];
			this.promises = [];
			try {
	...
	async deliverCall(func, thisArg) {
		try {
			...
			const result = Function.prototype.apply.call(func, thisArg, this.value);
	...
	disposeImpl(value, parent) {
		...
		switch (kind) {
			...
			case "rpc-target": {
				const target = value;
				const hook = this.rpcTargets?.get(target);
				if (hook) {
					hook.dispose();
					this.rpcTargets.delete(target);
				} else {
		...
		switch (kind) {
			...
			case "unsupported": {
				if (i === 0) {
					...
					const remainder = path.slice(0, i).join(".");
					throw new TypeError(
...
var ValueStubHook = class extends StubHook {
	...
	call(path, args) {
		try {
			const { value, owner } = this.getValue();
			const followResult = followPath(value, void 0, path, owner);
			...
			const promise = args.deliverCall(followResult.value, followResult.parent);
...
var PayloadStubHook = class _PayloadStubHook extends ValueStubHook {
	static {
		__name(_PayloadStubHook, "_PayloadStubHook");
	}
	constructor(payload) {
		super();
	...
	dup() {
		const thisPayload = this.getPayload();
...
var TargetStubHook = class _TargetStubHook extends ValueStubHook {
	static {
		__name(_TargetStubHook, "_TargetStubHook");
	}
...
var PromiseStubHook = class _PromiseStubHook extends StubHook {
	static {
		__name(_PromiseStubHook, "_PromiseStubHook");
	}
	promise;
	resolution;
	constructor(promise) {
	...
	stream(path, args) {
		args.ensureDeepCopied();
		const promise = this.promise.then((hook) => {
			const result = hook.stream(path, args);
...
var Devaluator = class _Devaluator {
	static {
		__name(_Devaluator, "_Devaluator");
	}
	...
	static devaluate(value, parent, exporter = NULL_EXPORTER, source) {
		const devaluator = new _Devaluator(exporter, source);
		try {
		...
		switch (kind) {
			...
			case "request": {
				...
				if (req.body) {
					...
					const hook = streamImpl.createReadableStreamHook(readable);
					const importId = this.exporter.createPipe(readable, hook);
			...
			case "response": {
				const resp = value;
				const body = this.devaluateImpl(resp.body, resp, depth + 1);
				const init = {};
			...
			case "error": {
				let e = value;
				const rewritten = this.exporter.onSendError(e);
				if (rewritten) {
					e = rewritten;
				}
				const result = ["error", e.name, e.message];
			...
			case "rpc-promise": {
				...
				const importId = this.exporter.getImport(hook);
				if (importId !== void 0) {
					if (pathIfPromise) {
						if (pathIfPromise.length > 0) {
			...
			case "rpc-target": {
				...
				const hook = this.source.getHookForRpcTarget(value, parent);
			...
			case "writable": {
				...
				const hook = this.source.getHookForWritableStream(value, parent);
			...
			case "readable": {
				...
				const ws = value;
				const hook = this.source.getHookForReadableStream(ws, parent);
				const importId = this.exporter.createPipe(ws, hook);
...
function fixBrokenRequestBody(request, body) {
	const promise = new Response(body).arrayBuffer().then((arrayBuffer) => {
		const bytes = new Uint8Array(arrayBuffer);
		const result = new Request(request, { body: bytes });
...
var Evaluator = class _Evaluator {
	...
	promises = [];
	evaluate(value) {
		const payload = RpcPayload.forEvaluate(this.hooks, this.promises);
		try {
			...
			if (value.length == 1 && value[0] instanceof Array) {
				...
				switch (value[0]) {
					...
					case "date":
						...
						break;
					case "bytes": {
						const b64 = Uint8Array;
						if (typeof value[1] == "string") {
							if (b64.fromBase64) {
								return b64.fromBase64(value[1]);
							} else {
								const bs = atob(value[1]);
								const len = bs.length;
					...
					case "error":
						if (
							...
							const cls = ERROR_TYPES[value[1]] || Error;
							const result = new cls(value[2]);
					...
					case "request": {
						...
						if (init.body instanceof ReadableStream && result.body === void 0) {
							const promise = fixBrokenRequestBody(result, init.body);
					...
					case "response": {
						...
						const body = this.evaluateImpl(value[1], parent, property);
						if (
							body === null ||
							typeof body === "string" ||
					...
					case "pipeline": {
						...
						const hook = this.importer.getExport(value[1]);
						if (!hook) {
							throw new Error(`no such entry on exports table: ${value[1]}`);
						}
						const isPromise = value[0] == "pipeline";
						const addStub = /* @__PURE__ */ __name((hook2) => {
							if (isPromise) {
								const promise = new RpcPromise(hook2, []);
								this.promises.push({ promise, parent, property });
								return promise;
							} else {
						...
						const subEval = new _Evaluator(this.importer);
					...
					case "remap": {
						...
						const captures = value[3].map((cap) => {
							if (
							...
							if (cap[0] === "export") {
								...
								const exp = this.importer.getExport(cap[1]);
								if (!exp) {
						...
						const instructions = value[4];
						const resultHook = hook.map(path, captures, instructions);
						const promise = new RpcPromise(resultHook, []);
						this.promises.push({ promise, parent, property });
						return promise;
					}
					case "export":
					case "promise":
						if (typeof value[1] == "number") {
							if (value[0] == "promise") {
								const hook = this.importer.importPromise(value[1]);
								const promise = new RpcPromise(hook, []);
								this.promises.push({ parent, property, promise });
								return promise;
							} else {
								const hook = this.importer.importStub(value[1]);
								this.hooks.push(hook);
								return new RpcStub(hook);
							}
						}
					...
					case "writable":
						if (typeof value[1] == "number") {
							const hook = this.importer.importStub(value[1]);
							const stream = streamImpl.createWritableStreamFromHook(hook);
							this.hooks.push(hook);
							return stream;
						}
						break;
					case "readable":
						if (typeof value[1] == "number") {
							const stream = this.importer.getPipeReadable(value[1]);
							const hook = streamImpl.createReadableStreamHook(stream);
							this.hooks.push(hook);
							return stream;
						}
						break;
...
var ImportTableEntry = class {
	...
	resolve(resolution) {
		...
		if (this.onBrokenRegistrations) {
			for (const i of this.onBrokenRegistrations) {
				const callback = this.session.onBrokenCallbacks[i];
				const endIndex = this.session.onBrokenCallbacks.length;
	...
	onBroken(callback) {
		if (this.resolution) {
			...
			const index = this.session.onBrokenCallbacks.length;
...
var RpcImportHook = class _RpcImportHook extends StubHook {
	static {
		__name(_RpcImportHook, "_RpcImportHook");
	}
...
var RpcSessionImpl = class {
	...
	exportStub(hook) {
		...
		const existingExportId = this.reverseExports.get(hook);
	...
	ensureResolvingExport(exportId) {
		const exp = this.exports[exportId];
		if (!exp) {
			throw new Error(`no such export ID: ${exportId}`);
		}
		if (!exp.pull) {
			const resolve = /* @__PURE__ */ __name(async () => {
				...
				for (;;) {
					...
					if (payload.value instanceof RpcStub) {
						const { hook: inner, pathIfPromise } = unwrapStubAndPath(
							payload.value,
						);
						if (pathIfPromise && pathIfPromise.length == 0) {
			...
			const autoRelease = exp.autoRelease;
			++this.pullCount;
			exp.pull = resolve()
				.then(
					(payload) => {
						const value = Devaluator.devaluate(
							payload.value,
							void 0,
							this,
							payload,
	...
	importPromise(idx) {
		...
		const entry = new ImportTableEntry(this, idx, true);
		this.imports[idx] = entry;
		return new RpcImportHook(
			/*isPromise=*/
			true,
	...
	getPipeReadable(exportId) {
		...
		const readable = entry.pipeReadable;
	...
	createPipe(readable, readableHook) {
		...
		const hook = new RpcImportHook(
			/*isPromise=*/
			false,
			entry,
		);
		const writable = streamImpl.createWritableStreamFromHook(hook);
		readable
			.pipeTo(writable)
			.catch(() => {})
	...
	sendCall(id, path, args) {
		if (this.abortReason) throw this.abortReason;
		const value = ["pipeline", id, path];
		if (args) {
			const devalue = Devaluator.devaluate(args.value, void 0, this, args);
			value.push(devalue[0]);
		}
		this.send(["push", value]);
		const entry = new ImportTableEntry(this, this.imports.length, false);
		this.imports.push(entry);
		return new RpcImportHook(
			/*isPromise=*/
			true,
	...
	sendStream(id, path, args) {
		...
		const devalue = Devaluator.devaluate(args.value, void 0, this, args);
		value.push(devalue[0]);
		const size = this.send(["stream", value]);
		const importId = this.imports.length;
		const entry = new ImportTableEntry(
			this,
			importId,
			/*pulling=*/
			true,
		...
		this.imports.push(entry);
		const promise = entry.awaitResolution().then(
			(p) => {
				p.dispose();
	...
	sendMap(id, path, captures, instructions) {
		...
		const devaluedCaptures = captures.map((hook) => {
			const importId = this.getImport(hook);
			if (importId !== void 0) {
				return ["import", importId];
			} else {
		...
		const value = ["remap", id, path, devaluedCaptures, instructions];
		this.send(["push", value]);
		const entry = new ImportTableEntry(this, this.imports.length, false);
		this.imports.push(entry);
		return new RpcImportHook(
	...
	abort(error, trySendAbortMessage = true) {
		...
		for (const i in this.onBrokenCallbacks) {
			try {
		...
		while (!this.abortReason) {
			...
			if (msg instanceof Array) {
				switch (msg[0]) {
					case "push":
						if (msg.length > 1) {
							const payload = new Evaluator(this).evaluate(msg[1]);
					...
					case "pipe": {
						...
						const hook = streamImpl.createWritableStreamHook(writable);
					...
					case "reject": {
						...
						if (typeof importId == "number" && msg.length > 2) {
							const imp = this.imports[importId];
							if (imp) {
								if (msg[0] == "resolve") {
									imp.resolve(
									...
									const payload = new Evaluator(this).evaluate(msg[2]);
									payload.dispose();
	...
	getStats() {
		const result = { imports: 0, exports: 0 };
		for (const i in this.imports) {
			++result.imports;
		}
		for (const i in this.exports) {
...
async function newHttpBatchRpcResponse(request, localMain, options) {
	...
	const batch = body === "" ? [] : body.split("\n");
	const transport = new BatchServerTransport(batch);
	const rpc = new RpcSession(transport, localMain, options);
...
var MapBuilder = class {
	...
	pushCall(hook, path, params) {
		...
		devalued = devalued[0];
		const subject = this.capture(hook.dup());
	...
	capture(hook) {
		...
		if (result === void 0) {
			if (this.context.parent) {
				const parentIdx = this.context.parent.capture(hook);
...
mapImpl.sendMap = (hook, path, func) => {
	const builder = new MapBuilder(hook, path);
	let result;
	try {
	...
	apply(instructions) {
		try {
			...
			for (const instruction of instructions.slice(0, -1)) {
				const payload = new Evaluator(this).evaluateCopy(instruction);
				if (payload.value instanceof RpcStub) {
					const hook = unwrapStubNoProperties(payload.value);
					if (hook) {
...
function applyMapToElement(input, parent, owner, captures, instructions) {
	const inputHook = new PayloadStubHook(
		RpcPayload.deepCopyFrom(input, parent, owner),
	);
	const mapper = new MapApplicator(captures, inputHook);
	try {
...
var WritableStreamStubHook = class _WritableStreamStubHook extends StubHook {
	static {
		__name(_WritableStreamStubHook, "_WritableStreamStubHook");
	}
	state;
	...
	call(path, args) {
		try {
			...
			const func = state.writer[method];
			const promise = args.deliverCall(func, state.writer);
			return new PromiseStubHook(
...
var FlowController = class {
	...
	onAck(token) {
		const ackTime = this.now();
		this.delivered += token.size;
		this.deliveredTime = ackTime;
		this.bytesInFlight -= token.size;
		const rtt = ackTime - token.sentTime;
		...
		if (this.firstAckTime === 0) {
			...
			const interval = ackTime - baseTime;
			const bytes = this.delivered - baseDelivered;
			const bandwidth = bytes / interval;
			const growthFactor = this.inStartupPhase
...
function createWritableStreamFromHook(hook) {
	...
	const fc = new FlowController(() => performance.now());
	let windowResolve;
	let windowReject;
	...
	return new WritableStream({
		write(chunk, controller) {
			...
			if (size === void 0) {
				...
				const { token, shouldBlock } = fc.onSend(size);
				promise.then(
					() => {
						const hasCapacity = fc.onAck(token);
...
var ReadableStreamStubHook = class _ReadableStreamStubHook extends StubHook {
	static {
		__name(_ReadableStreamStubHook, "_ReadableStreamStubHook");
	}
	state;
	// undefined when disposed
...
async function newWorkersRpcResponse(request, localMain) {
	if (request.method === "POST") {
		const response = await newHttpBatchRpcResponse(request, localMain);
...
// .wrangler/tmp/bundle-DtGa0S/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-DtGa0S/middleware-loader.entry.ts
