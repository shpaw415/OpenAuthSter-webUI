...
export async function GET(): Promise<{
  ...
  try {
    ...
    const webhooks = await db.select().from(WebHookTable);
    ...
    const logs24hRows = await db
      ...
      .from(LogsTable)
      .where(gt(LogsTable.timestamp, oneDayAgo));
